...
void awake_waiting_tasks(struct task_t *task) {
	...
	while (item) {
		#ifdef DEBUG
		ppos_debug("acordando item esperando\n");
		...
		if (executar_task) {
			...
			switch(executar_task->status) {
				...
				case TASK_TERMINATED:
					#ifdef DEBUG
					ppos_debug("tarefa terminada. limpando...\n");
					#endif
...
void task_suspend(struct queue_t *queue) {
	...
	#ifdef DEBUG
	ppos_debug("trocando pro kernel\n");
	#endif
	task_switch(task_kernel);
}
...
int task_wait(struct task_t *task) {
	...
	#ifdef DEBUG
	ppos_debug("tarefa %s ira esperar por %s\n", task_name(task_atual), task_name(task));
	#endif
